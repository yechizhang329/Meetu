...
- **不夸大不编造**：不编造故事、数据、包含项
- **不用"搭子"**：全平台去"搭子"，用自然语言替代
- **品牌语气灰度70-80%**：有态度同龄人，偏有趣/自由
- **数据驱动检查清单**：所有帖子在发布前必须通过 `xhs-content-checklist` 检查清单（步骤5.5）
...
## 步骤 5.5：数据驱动检查清单（必须通过）

**目标**：确保每条内容针对算法三类核心数据做了优化。

**必须执行**：在展示给用户之前，按 `xhs-content-checklist` skill的检查清单逐项过一遍。

检查三大维度：
1. **吸引力**（A1-A5）：封面可识别、标题给确定感、第一句最有价值、标题≤20字、封面醒目
2. **互动性**（B1-B6）：有金句、有评论钩子、有收藏价值、有破绽/争议、自评已设计、身份标签感
3. **停留**（C1-C3）：无冗余、每段新信息、正文≤1000字
4. **合规**（D1-D9）：无敏感词、非产品介绍帖、无夸大编造、无禁用词"搭子"

**如有不通过项：必须先修改内容，再重新检查，直到全部通过才进入Review。**

输出格式参照 `xhs-content-checklist` skill。
...
完成步骤 4+5+5.5 后，**必须先展示完整内容+检查清单结果给用户确认**，格式如下：
